...
export function ContactPanel() {
  ...
  const contactSession = useQuery(api.private.contactSessions.getOneByConversationId, conversationId ? { conversationId } : "skip");
    ...
    return [
      {
        ...
        items: [
          ...
          {
            label: "Language",
            value: contactSession.metadata.lenguage,
          },
          {
            label: "Timezone",
        ...
        items: [
          {
            ...
            value: new Date(contactSession._creationTime).toLocaleString(undefined, {
              year: 'numeric',
              month: 'short',
              day: 'numeric',
              hour: 'numeric',
    ...
    <div className='flex h-full w-full flex-col bg-background text-foreground'>
      <div className="flex flex-col gap-y-4 p-4">
        <div className="flex items-center gap-x-2">
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <p className="line-clamp-1 text-sm text-muted-foreground">{contactSession.email}</p>
          </div>
        </div>
      ...
      <div>
        {contactSession.metadata && (
          <Accordion className="w-full rounded-none border-y" collapsible type="single" defaultValue={accordionSections[0]?.id}>
            {accordionSections.map((section) => (
              <AccordionItem key={section.id} value={section.id} className='rounded-none outline-none has-focus-visible:z-10 has-focus-visible:border-ring has-focus-visible:ring-ring/50 has-focus-visible:ring-[3px]'>
                  ...
                  <div className="space-y-2 text-sm">
                    {section.items.map((item) => (
                      <div className="flex justify-between" key={`${section.id}>${item.label}`}>
                        ...
                        <span className={cn(item.className)}>{item.value}</span>
                      </div>
                    ))}
                  </div>
                </AccordionContent>
